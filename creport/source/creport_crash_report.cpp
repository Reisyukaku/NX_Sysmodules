...
namespace ams::creport {
    ...
    namespace {
        ...
        void TryCreateReportDirectories() {
            fs::EnsureDirectoryRecursively("sdmc:/ReiNX/crash_reports/dumps");
            fs::EnsureDirectoryRecursively("sdmc:/ReiNX/fatal_reports/dumps");
        }
    ...
    void CrashReport::SaveReport() {
        ...
        /* Save files. */
        {
            ...
            /* Save crash report. */
            std::snprintf(file_path, sizeof(file_path), "sdmc:/ReiNX/crash_reports/%011lu_%016lx.log", timestamp, this->process_info.program_id);
            {
            ...
            /* Dump threads. */
            std::snprintf(file_path, sizeof(file_path), "sdmc:/ReiNX/crash_reports/dumps/%011lu_%016lx_thread_info.bin", timestamp, this->process_info.program_id);
            {
                ScopedFile file(file_path);
                if (file.IsOpen()) {
