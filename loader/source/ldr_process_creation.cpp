...
#undef COPY_ACCESS_CONTROL
        ...
        Result LoadNsoIntoProcessMemory(Handle process_handle, fs::FileHandle file, uintptr_t map_address, const NsoHeader *nso_header, uintptr_t nso_address, size_t nso_size, const ncm::ProgramId program_id) {
            /* Map and read data from file. */
            {
                ...
                /* Apply IPS patches. */
                LocateAndApplyIpsPatchesToModule(nso_header->build_id, map_address, nso_size, program_id);
            }
        ...
        Result LoadNsosIntoProcessMemory(const ProcessInfo *process_info, const NsoHeader *nso_headers, const bool *has_nso, const args::ArgumentInfo *arg_info, const ncm::ProgramId program_id) {
            ...
            for (size_t i = 0; i < Nso_Count; i++) {
                if (has_nso[i]) {
                    ...
                    R_TRY(LoadNsoIntoProcessMemory(process_handle, file, map_address, nso_headers + i, process_info->nso_address[i], process_info->nso_size[i], program_id));
                }
            }
    ...
    Result CreateProcess(Handle *out, PinId pin_id, const ncm::ProgramLocation &loc, const cfg::OverrideStatus &override_status, const char *path, u32 flags, Handle reslimit_h) {
...

        {
            ...
            R_TRY(LoadNsosIntoProcessMemory(&info, nso_headers, has_nso, arg_info, loc.program_id));
