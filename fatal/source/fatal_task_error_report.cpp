...
namespace ams::fatal::srv {
    ...
    namespace {
        ...
        void TryEnsureReportDirectories() {
            mkdir("sdmc:/ReiNX", S_IRWXU);
            mkdir("sdmc:/ReiNX/fatal_reports", S_IRWXU);
            mkdir("sdmc:/ReiNX/fatal_reports/dumps", S_IRWXU);
        }
        ...
        void ErrorReportTask::SaveReportToSdCard() {
            ...
            /* Open report file. */
            snprintf(file_path, sizeof(file_path) - 1, "sdmc:/ReiNX/fatal_reports/%011lu_%016lx.log", timestamp, static_cast<u64>(this->context->program_id));
            FILE *f_report = fopen(file_path, "w");
            if (f_report != NULL) {
                ON_SCOPE_EXIT { fclose(f_report); };

                fprintf(f_report, "ReiNX Fatal Report (v1.0):\n");
            ...
            if (this->context->stack_dump_size) {
                snprintf(file_path, sizeof(file_path) - 1, "sdmc:/ReiNX/fatal_reports/dumps/%011lu_%016lx.bin", timestamp, static_cast<u64>(this->context->program_id));
