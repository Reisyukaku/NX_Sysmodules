...
namespace ams::boot {
    ...
    namespace {
        ...
        void DoRebootToPayload(ams::FatalErrorContext *ctx) {
            ...
            /* Copy in payload. */
            for (size_t ofs = 0; ofs < ReiNX_bin_size; ofs += sizeof(g_work_page)) {
                std::memcpy(g_work_page, &ReiNX_bin[ofs], std::min(static_cast<size_t>(ReiNX_bin_size - ofs), sizeof(g_work_page)));
